...
const TIKTOK_OAUTH_EXCHANGE_URL = "https://qtjwzfbinsrmnvlsgvtw.supabase.co/functions/v1/tiktok-oauth-exchange";
  ...
  useEffect(() => {
    const handleCallback = async () => {
      ...
      try {
        ...
        const response = await fetch(TIKTOK_OAUTH_EXCHANGE_URL, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            "Authorization": `Bearer ${session.access_token}`,
          },
          body: JSON.stringify({ auth_code: authCode, redirect_uri: redirectUri }),
        });
        ...
        setStatus("success");
        toast.success(`✅ ${t("connections.tiktokSuccess", { count: result.accountsCount || 0 })}`);
      ...
      <div className="glass rounded-xl p-8 max-w-md w-full mx-4 text-center">
        {status === "processing" && (
          <>
            ...
            <h2 className="text-xl font-semibold text-foreground mb-2">{t("connections.processingTikTok")}</h2>
            <p className="text-muted-foreground">{t("connections.processingTikTokDesc")}</p>
          </>
        )}
        {status === "success" && (
          <>
            ...
            <p className="text-muted-foreground">{t("connections.successTikTok")}</p>
          </>
        )}
        {status === "error" && (
          <>
